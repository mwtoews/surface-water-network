...
def test_read_write_formatted_frame(tmp_path):
    ...
    # without index
    swn.file.write_formatted_frame(df1, fname, comment_header=True, index=False)
    lines = fname.read_text().splitlines()
    assert len(lines) == 7
    ...
    swn.file.write_formatted_frame(df1, fname, comment_header=False, index=False)
    lines = fname.read_text().splitlines()
    assert len(lines) == 7
    ...
    assert lines[0] == "# rno value1 value2 value3"
    df2 = swn.file.read_formatted_frame(fname).set_index("rno")
    pd.testing.assert_frame_equal(df, df2, check_index_type=False, check_dtype=False)
    ...
    pd.testing.assert_frame_equal(
        df.reset_index(drop=True), df2, check_index_type=False, check_dtype=False
    )
    ...
    swn.file.write_formatted_frame(df, fname, comment_header=False, index=False)
    lines = fname.read_text().splitlines()
    assert len(lines) == 1
    assert lines[0] == "value1 value2 value3"
    df2 = swn.file.read_formatted_frame(fname)
    ...
    swn.file.write_formatted_frame(df, fname, comment_header=True, index=True)
    header = fname.read_text().splitlines()[0]
    assert header == "# index  k  i   j  elev"
    df2 = swn.file.read_formatted_frame(fname)
    assert df.index.name is None
    df.index.name = "index"
    pd.testing.assert_frame_equal(df, df2.set_index("index"), check_dtype=False)

    swn.file.write_formatted_frame(df, fname, comment_header=True, index=False)
    header = fname.read_text().splitlines()[0]
